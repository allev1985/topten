...
/**
 * Dialog for editing a place's name and address from the My Places page.
 *
 * Ownership is verified in the service layer via places.userId
 * (no listId required in this context).
 *
 * Dirty-state tracking mirrors the list-detail EditPlaceDialog:
 *   - Save disabled when form is clean or invalid.
 *   - "Unsaved changes" badge while dirty.
 *   - window.confirm() guard on close-with-dirty.
 */
export function EditPlaceDialog({
  ...
  // Sync initial ref when dialog opens for a (potentially) different place
  useEffect(() => {
    if (open) {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        <form action={formAction} className="space-y-4">
          {/* placeId only — no listId needed (ownership via places.userId) */}
